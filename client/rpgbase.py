...
from items import *
from avlib import htmlColor as color
...
SLOTS={'Head':1,'Body':1,'Legs':1,'Boots':1,'Gloves':2,'Neck':1,'Cloak':1,'Rings':6,'Hands':2,'Braces':2}
#SLOTS.extend(MSLOTS)
...
class Wearable(Item):
	...
	def genName(self):
		...
		if self.is_id:
			name='%s %s %s %s' % (color(self.quality['name'],self.quality['color']),self.title['name'],self.material['name'],itn)
		else:
...
HP: {hp}/{maxhp} Mana: {mana}/{maxmana} Stamina: {st}/{maxst}
		...
		""".format(title=self.Title,name=self.Name,level=self.Level,str=self.Strength,agl=self.Agility,spd=self.Speed,int=self.Intelegence,maxhp=self.MaxHP,hp=self.HP,maxmana=self.MaxMana,mana=self.Mana,defense=self.Defense,dmg=self.Damage,st=self.Stamina,maxst=self.MaxStamina)
...
def genItem(hero,**kwargs):
	...
	if slot=='Hands':
		...
		if 'item_type' in kwargs and kwargs['item_type'] in names(TYPES[slot],complex=True):
			...
			item_type=rand({'value':TYPES[slot]['Rare'],'p':0.1},{'value':TYPES[slot]['Unique'],'p':0.05},{'value':TYPES[slot]['Common'],'p':0.8})
	else:
	...
	if 'material' in kwargs and kwargs['material'] in names(MATERIALS[item_type['type']],complex=True):
		...
		material=rand({'value':MATERIALS[item_type['type']]['Rare'],'p':0.1},{'value':MATERIALS[item_type['type']]['Unique'],'p':0.05},{'value':MATERIALS[item_type['type']]['Common'],'p':0.8})
	...
	if 'quality' in kwargs and kwargs['quality'] in names(QTITLES,complex=True):
		...
		quality=rand({'value':QTITLES['Rare'],'p':0.1},{'value':QTITLES['Unique'],'p':0.05},{'value':QTITLES['Common'],'p':0.8})
	if 'title' in kwargs and kwargs['title'] in names(TITLES,complex=True):
		title=itemByType(TITLES,kwargs['title'],complex=True)
	else:
		title=rand({'value':TITLES['Rare'],'p':0.1},{'value':TITLES['Unique'],'p':0.05},{'value':TITLES['Common'],'p':0.8})
