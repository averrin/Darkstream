...
#        App.__init__(self,*args,**kwargs)
        ...
        self.app=app
        self.app.sm=self
        if self.app.settings['has_plugins']!='False':
            self.loadPlugins()
            ...
            self.tabWidget.removeTab(1) #tabPlugins settings

        self.loadSettings()
        self.connect(self.applyButton, SIGNAL("clicked()"), self.applyOptions)

    def loadSettings(self):
        self.opts=self.app.project.config['Options'].copy()
        if self.app.has_plugins:
    ...
    def applyOptions(self):
        self.app.project.config['Options']=self.opts
        if self.app.has_plugins:
...
#    print dir(logger)
    project=Project()
    logapi=logger.name(project.config['Settings']['name'])
    api=API()
    qtapp = QApplication(sys.argv)
    app=App(project,logapi=logapi,api=api)
